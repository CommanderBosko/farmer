...
| `NUM_DRONES` | Number of parallel drones (1–32); capped to `world_size`; drives the Hay/Wood/Carrot/Pumpkin grid split and the Cactus/Sunflower strip functions. Maze parallelism is separate (`MAZE_FOUR_CORNER`); Bones always runs single-drone. Requires Megafarm upgrade (default `32`) |
| `MAZE_FOUR_CORNER` | When `True`, gold/maze runs use a four-corner parallel solve (a solver per corner, nearest-to-treasure wins, losers bail when `num_items(Gold)` jumps) — measured ~2× faster than single-drone. `False` reverts to the original single-drone solve. Falls back to single-drone when `world_size < 2` (default `True`) |
...
- **Maze** — runs when **Gold** is the resource being farmed (Gold is the next unlock's bottleneck, or the lowest stock once all unlocks are maxed) and enough `Items.Weird_Substance` is stockpiled for one run; returned as `Items.Gold` from `plant_decision()` and dispatched to `farm_maze()`. With `config.MAZE_FOUR_CORNER` (default `True`), `farm_maze()` `clear()`s, walks a solver to each of the four corners *before* growing the maze (so they're placed on open ground — walls would block post-growth), grows the maze from the origin corner, then all four left-hand wall-follow at once (`maze_solver()`): the corner nearest `Entities.Treasure` reaches it first and harvests, and the others bail the instant `num_items(Items.Gold)` jumps (live-shared across drones — no slow-loser tail). Step-counter safety valve `world_size² × 4` per solver; `clear()` again to reset. With the flag off (or `world_size < 2`) it uses `farm_maze_single()`, the original single-drone wall-follow. Single-use only (no reuse stacking). Measured ~2× faster four-corner vs single (2026-06-19).
...
- Maze parallelizes via the four-corner solve (`MAZE_FOUR_CORNER`, default on): one solver per corner racing the same maze, coordinating through live-shared `num_items(Items.Gold)` rather than globals (globals are *copied* per spawned drone, not shared). Cactus and Sunflower parallelize across drones via strip functions. Bones is single-drone.
